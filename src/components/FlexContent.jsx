...
const FlexContent = ({ ifExists, endpoint: { title, heading, text, img, btn, url } }) => {
  return (
   <>
      <div className={`flex items-center justify-between flex-col md:flex-row lg:justify-center nike-container ${ifExists ? 'flex-row-reverse' : 'flex-row'}`}>
        ...
        <div className='flex items-center justify-center max-w-xl relative lg:max-w-none w-full'>
          <img
            src={img}
            alt={`img/${heading}`}
            className={`w-auto p-6 object-fill transitions-theme ${ifExists ? 'lg:h-56 md:h-52 sm:h-44 h-36 rotate-6 hover:-rotate-12' : 'h-72 lg:h-64 md:h-60 sm:h-48 xsm:h-40 rotate-[19deg] hover:rotate-12'}`}
          />
        </div>
      </div>
   </>
